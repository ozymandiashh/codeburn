...
<p align="center">
  ...
  <a href="docs/providers/muse-code.md" title="Muse Code (Meta)">Muse Code</a>
...
| **Muse Code** (`muse-code`) | `~/.local/share/muse/sessions/YYYY/MM/DD/<session-id>/session.jsonl` plus `<session-id>/subagent/<child-id>/session.jsonl` (honors `XDG_DATA_HOME`; `MUSE_DATA_DIR` is an exact override) | Meta's `muse` CLI. Muse 1.1.x writes two records for one model step - a `goal_usage_attribution` carrying the tokens and a stable `usage_id`, and a `model_completed` carrying the model name - so CodeBurn pairs them and counts the step once, keyed on the usage id rather than on position. The same key collapses a delegated run mirrored into both the parent log and `subagent/`. `.msp-view-v1/` is a fold of the same usage and is skipped. No quota adapter: Muse does not persist plan windows to disk. |
