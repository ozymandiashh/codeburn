...
### Added (macOS)
- **A weekly or monthly quota window that lasts now says how much of it your pace leaves unused.** `Lasts until reset` answered whether a long window makes it, but not whether you are finishing the week at 98% or at 56% of what you pay for; the projection that answers that sat only in the tooltip. The caption under each Capacity Dock window column and under each bar in the agent-tab quota hover card now reads `Lasts until reset · ~44% unused` (5% used 15 hours into a 7-day window projects to 56% at reset), with "at this pace" and the projection in the tooltip. The share is the complement of the projection the tooltip already showed, rounded to a whole percent: one subtraction from a figure CodeBurn already printed, from the same `QuotaPace` math behind the Plan tab's `30% in reserve · 40% at reset`, not a second estimate. It appears only when the rounded share is at least 10 points and the window is outside the two-point on-pace band, because a single-digit leftover is noise on a linear model and would flicker in and out; below that the caption stays `Lasts until reset`. Everything that already silenced the caption still does (the first 3% of a window, a missing or unvalidated duration, stale, disconnected or older-than-ten-minute data), and windows of six hours or less keep their `On pace` / `in deficit` / `in reserve` stage unchanged. Translated for zh-Hans. `codeburn quota` prints no pace caption, so the CLI is unchanged. (#725)
