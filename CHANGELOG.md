...
### Added
- **`codeburn quota` estimates the chance of an OpenAI Codex usage-limit reset landing soon, and always prints the range around it.** A new Codex section reads `Reset forecast: 24% chance in the next 24h (10 to 51%), 4% in 6h (1 to 14%). 12h since the last global reset; typical wait 2.2d. Working hours in SF: yes.`, with the numbers also carried structurally under `providers[].resetForecast` in `--format json`, and the same two lines in the macOS Codex quota hover card and Plan tab. The model is a pure module, `src/reset-forecast.ts`, mirrored word for word in Swift because the menubar reads quota from its own adapters rather than the CLI payload: an empirical conditional survival estimate over the inter-reset waits, weighting the last 120 days three times as heavily because the cadence sped up, shrunk toward a memoryless rate where the at-risk set thins, ranged by a Wilson interval on the unweighted counts, and tilted by the observed hour-of-day density in San Francisco — where no reset in the record lands between 02:00 and 07:00 Pacific and 95% land between 07:00 and 23:00. The prior never takes a probability to exactly zero, no estimate is allowed to reach certainty, and a missing or unusable record produces a stated reason rather than a zero. `confidence` is `low` unless a walk-forward backtest beats the base rate; on the record that ships it does not, so it reports `low`, and a test asserts that rather than letting the label be claimed. A record older than 14 days is called stale on the line itself. The dataset is `src/data/codex-reset-history.json`, 44 resets and 9 credit grants from the public codex-reset.com timeline, kept as ids, instants and a type with every text field dropped, refreshed every six hours by `.github/workflows/refresh-codex-reset-history.yml` into a pull request and never by the client: #725's no-new-polling non-goal holds in full, under the maintainer's written exception that the dataset is refreshed by a GitHub Action in the repo, never by the client at runtime. `docs/codex-reset-forecast.md` explains the model and its limits. (#725)
...
- **An opt-in notification when a Codex reset starts to look likely, off by default.** Settings → General → Notifications switches it on and picks a threshold, defaulting to a 50% chance within six hours. It fires at most once per crossing and re-arms only once the estimate falls back under the threshold, the fired state is persisted to `codex-reset-forecast.json` in the CodeBurn cache directory so relaunching cannot repeat it, and a new reset or a moved threshold starts a new crossing. A failed refresh is no opinion rather than a re-arm, and a stale record or an estimate already past the end of the record never fires at all. The text says what it is before it says a number — "A statistical estimate from public reset history, not an announcement from OpenAI" — and it carries no button, no link and no side effect: nothing in this feature spends, redeems, requests or refreshes anything. It is the only quota notification in CodeBurn that defaults to off, because the others report something that already happened and this one reports a probability. (#725)
- **The reset history refreshes itself from GitHub, so the forecast is no longer reading a record that is a release old.** The dataset is now published every 30 minutes to a `data/codex-reset-history` branch, and the client asks for it at most once an hour with a conditional GET — `api.github.com`, the same host the app already uses for update checks, with the stored ETag, so the usual answer is a 304 with no body and one request against the unauthenticated hourly limit. It rides the Codex quota refresh that already runs, never blocks anything, and only happens while Codex is connected. Nothing about the account goes with it: no credential, no cookie, no plan, no usage. A fetched record is adopted only if it passes the same validation the workflow applies and is strictly newer than what is held, so a malformed, truncated, reordered or text-carrying answer changes nothing; offline, rate-limited or switched off, the record compiled into the build is used exactly as before. Settings → General → Reset Forecast Data has a switch, on by default, beside a line naming which copy is in use and when it was built; `codeburn quota` honours `CODEBURN_PRICING_SNAPSHOT_ONLY` and prints which copy it read and when that copy was built. The client still never contacts codex-reset.com. (#725)
- **The forecast's "since last reset" clock follows resets this machine saw for itself, not just the bundled record.** The distribution of waits can be days old without harm — 44 waits do not change shape in a week — but the time since the last reset cannot, so it is read from what the early-reset detector and the banked-credit watcher persist: a reset that lands on this machine moves the clock within one quota refresh cycle instead of waiting for the next release. The stores are read, not imported, through private mirror types, so neither feature has to be installed: an absent file, an absent key, a wrong shape or an unreadable date is no opinion, and the forecast falls back to the global record exactly as before. An early reset of an Anthropic window never moves the Codex clock. When a local reset is the newer one the sentence says so — "12h since the reset observed on this machine at 14:30" rather than "since the last global reset". `codeburn quota` has no persisted quota store, so the CLI stays conditioned on the bundled record. (#725)
