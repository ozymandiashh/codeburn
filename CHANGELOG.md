...
### Added
- **CodeBurn reads Meta's Muse Code (`muse`) sessions.** The new `muse-code` provider walks `$XDG_DATA_HOME/muse/sessions/YYYY/MM/DD/<session-id>/session.jsonl` (default `~/.local/share/muse`, with `MUSE_DATA_DIR` as an exact override the way `OPENCODE_DATA_DIR` is) together with the `subagent/<child-id>/session.jsonl` transcripts beside it, and prices each model step from the Muse Spark rows already in the bundled LiteLLM snapshot, contributor tier included. Two double counts are handled by name: Muse 1.1.x writes both a `goal_usage_attribution` and a `model_completed` record for one model step, so the two are paired and counted once with the tokens and the stable `usage_id` coming from the first and the model name from the second; and a delegated run is copied into the parent's log as well as its own, so every call is deduplicated on that usage id rather than on position, which also collapses a forked session replaying the prefix it inherited. The `sessions/.msp-view-v1/` fold of the same usage is skipped, the retained-frame transaction wrapper that opens every transcript is unwrapped, microsecond timestamps are read as such, reasoning tokens are treated as already inside output per Meta's own wire schema, and a step whose model cannot be read is reported unpriced rather than guessed into one of the two tiers, which are 12.5x apart. The model itself is resolved in precedence order — the completion's own `model`, then another completion in the same run, then the session default, which starts at the metadata record's `model_id` and moves with `model_selection_initialized`, `model_reconfigure_completed` and `run_model_configured` while a failed, rejected or unserved reconfigure deliberately leaves it alone — so a session that switches models prices its legs apart. `probeRoots()` reports the sessions dir to `codeburn doctor` so a missing install never reads as a silent $0.00 (#899). There is no quota adapter: Muse does not persist its plan windows to disk, and reading them would cost a billed model call per refresh. Validated against a real Muse Code 1.1.1 binary's own logs plus published excerpts of real Meta-provider sessions; see `docs/providers/muse-code.md` for exactly which fields are confirmed from which source.
