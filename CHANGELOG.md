...
- **An opt-in notification when a Codex reset starts to look likely, off by default.** Settings → General → Codex Reset Forecast switches it on and picks a threshold, defaulting to a 50% chance within six hours. It fires at most once per crossing and re-arms only once the estimate falls back under the threshold, the fired state is persisted to `codex-reset-forecast.json` in the CodeBurn cache directory so relaunching cannot repeat it, and a new reset or a moved threshold starts a new crossing. A failed refresh is no opinion rather than a re-arm, and a stale record or an estimate already past the end of the record never fires at all. The text says what it is before it says a number — "A statistical estimate from public reset history, not an announcement from OpenAI" — and it carries no button, no link and no side effect: nothing in this feature spends, redeems, requests or refreshes anything. It is the only quota notification in CodeBurn that defaults to off, because the others report something that already happened and this one reports a probability. (#725)
