...
### Added (Windows)
- **The Windows tray app speaks Simplified Chinese, seeded from the menubar's own glossary.** Every user-facing string in the tray menu, the tooltip, the popover, the Capacity Dock, the update surfaces and all of Settings now resolves through one lookup helper per runtime — `l()` in Rust, `L()` in the webview, both the shape of the menubar's `L(_:)` — against one pair of catalogs under `windows/src/i18n/`, `en.lproj` an identity table and `zh-Hans.lproj` seeded verbatim from `mac/Sources/CodeBurnMenubar/Resources/zh-Hans.lproj/Localizable.strings` (620 keys), so a sentence both apps render is one key with one translation everywhere: Today 今天, Week 本周, Month 本月, Cost 花费, Calls 调用, Sessions 会话, Capacity Dock 容量 Dock, Settings 设置. The 211 keys only the tray renders are translated onto that vocabulary and live in marked sections at the end of each file; the Rust side embeds the very same files with `include_str!`, so the menu and the webview cannot disagree about a term. Keys are the English copy, so a missing translation degrades to correct English; provider, model and plan names, units, currency codes, shell commands and anything the `codeburn` CLI produces stay verbatim; and format specifiers (`%@`, `%lld`, `%%`) are a contract between the two tables, in order. Settings → General gains a Language picker (System / English / 简体中文) persisted as `language` in `windows-settings.json` beside the other tray preferences; System follows the Windows UI language (`GetUserDefaultUILanguage`, every Chinese variant resolving to the zh-Hans table, the same fallback AppKit makes for a zh-Hant user), and because the Rust tray menu is built before any webview exists the choice applies on the next launch — the picker says "Relaunch to apply." and offers the button, as the mac's does. Day and month names, weekday initials and the calendar title follow the resolved language through `Intl`, keeping the existing English output byte-identical. The one truncation the tray performs — the fetch-error overlay's message cap — now counts display cells rather than characters, porting `displayCells`/`abbreviate` from `MenubarSecondRow.swift` (a Han glyph counts twice, so `6 小时 2 分` is 8 characters and 11 cells) with the 24-cell row budget available to both runtimes for any future bound. A `cargo test` suite mirrors the menubar's `LocalizationCatalogTests`: it fails the build when the tables disagree on keys, leave a value blank, break the en identity, mismatch argument specifiers or literal percents, or when a key carries no sentence to translate, and it pins spot translations so the shared glossary cannot silently drift. This is the Windows half of the localization work only — the CLI and the web dashboard are untouched. (#1336)
