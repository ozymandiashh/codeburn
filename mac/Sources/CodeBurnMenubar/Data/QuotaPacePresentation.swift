...
enum QuotaPacePresentation {
    ...
    static func caption(for result: QuotaPace.Result, windowSeconds: Int, now: Date = Date()) -> String {
        ...
        if compact {
            ...
            if result.deltaPercent > 0 { return L("%lld%% in deficit", Int(result.deltaPercent.rounded())) }
            return L("%lld%% in reserve", Int(-result.deltaPercent.rounded()))
        }
    ...
    static func countdownLabel(seconds: TimeInterval) -> String {
        ...
        let days = hours / 24
        // The popover countdown's own keys, so the two cannot disagree.
